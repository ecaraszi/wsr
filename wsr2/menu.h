...
struct menu_t : public state_t {
  ...
  virtual void start() override { selected = 0; }
  ...
  virtual void loop() override {
    ...
    int low = 0;
    int high = max(0, item_count - 1);
    selected = max(low, min(high, selected + knob.diff));
    ...
    if (button_knob.pushed() || button_yes.pushed()) {
      if (items[selected]->state != state_t::ITEM_ONLY) {
